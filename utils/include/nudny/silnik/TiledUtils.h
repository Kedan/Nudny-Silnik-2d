...
namespace nudny {
	...
	class TiledUtils {
		public:
			 ...
			 */		
			static void LoadSprite( Sprite& t_sprite, tson::Tile& t_tile, std::shared_ptr<Resources> t_resources, std::string t_dir, glm::vec2 t_map_grid_size );
			 ...
			 * @param	glm::vec2			- map grid size. Used to correct tile size and tile drawing rectangle size.
			 */
			static void LoadTileset( Tileset& t_tileset, tson::Tileset& t_tson_tileset,  std::shared_ptr<Resources> t_resources, std::string t_dir, glm::vec2 t_map_grid_size );

			static bool LoadMap( Map& t_map, std::string t_map_file );

			static bool LoadMapAssets( Map& t_map, tson::Map& t_tson_map );

			static bool LoadMapLayers( Map& t_map, tson::Map& t_tson_map );
	}; // class TiledUtils
}; // namespace
