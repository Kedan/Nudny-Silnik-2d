...
int App::Run() {
	...
	while( m_window.isOpen() && !m_exit ) {
		...
		while( m_window.pollEvent( m_event )) {
			...
			m_game.Events( m_event );
		//	for( auto interactive_obj : m_interactive_list ) {
		//		interactive_obj.second->Events( m_event );
		//	}
			// layers.interactives.Events( m_event );
		}
		m_window.clear( sf::Color::Black );
		m_game.Update( m_time.asSeconds() );
	...
	//	}
		// layers.updatables.Update( m_time.asSeconds() );
		m_window.setView( m_view );
		m_game.Draw( m_window );	
	//	for( auto dobj : m_draw_list ) {
	//		dobj->Draw( m_window );
	//	}
		// layers.drawables.Draw( m_window );	
		m_window.display();
	}
	m_window.close();
	LOG.Time();
